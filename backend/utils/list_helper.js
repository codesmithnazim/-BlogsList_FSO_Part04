...
//     const mostLikes=  Math.max(...blogs.map(each=> each.blogs))
//     const completeBlog= blogs.find(each=> each.likes===mostLikes)
//     return {author:completeBlog.author, likes: completeBlog.likes}
// }
...
function favoriteBlogger(blogs) {
    const newArray= blogs.map(each=>{ return  {'author': each.author, 'likes': each.likes}})  
    let maxLikes=0
    ...
    for (const index of newArray) {
        recordingObj[index.author]= (recordingObj[index.author] || 0)+ index.likes
    ...
    logger.info("Recording Object from favoriteBlogger()", recordingObj)
    return  finalObj
}
...
export default { dummy, totalLikes ,favoriteBlog, mostBlogs , favoriteBlogger};
